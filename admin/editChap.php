...
require_once('function/function.php');
	...
	if(isset($_SESSION['email'])){
		...
		}elseif($db->GetLevelUser($_SESSION['email'])<1){
			 $menu_user_up=1;
			 $user= $_SESSION['email'];
		}
		else{
              ...
              <div class="form-group">
               ...
               <div class="form-group">
                <label for="inputClientCompany">Tiêu đề chương</label>
               		<input type="text" class="form-control"  id="Title" name="Title" value="<?=$arr_Chapter[3]?>"/>
              </div>
            </div>
            <!-- /.card-body -->
          </div>
...
$(function () {
  ...
  $(document).ready(function(){ 
	 ...
	 $.ajax({     
	       ...
	       type:'POST',
	       cache:false,
	       data:{'idStory':idStory,'name':name,'site':site,'Title':Title,'link':temp3.toString()},
	       success:function(kq){
	       	var o = JSON.parse(kq);
	                 //console.log(kq);
	                 //alert(o.content);
				window.open("previewChap.php?idStory="+o.idStory+"&name="+o.name+"&site="+o.site+"&Title="+o.Title+"&link="+o.link, '_blank');
