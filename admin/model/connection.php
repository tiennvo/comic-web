...
class config
{
	...
	function GetChapAdmin($key,$per_page,$page,$type,$idStory)
	{
		...
		if($type==0)
			$l="LIMIT ".$per_page." OFFSET ".$offset;
		   $sql = "SELECT * from qq_chapter WHERE `IdStory`='$idStory' and concat(`Name`,`Summary`,`DateUpload`) like '%$key%' ".$l;
	...
	function AddStory($Name,$NameOther,$Status,$Content,$Avatar,$Badge,$Waning,$Author,$Genre,$NameEncodeGenres,$Country,$DateUpload,$URL1,$URL2,$female,$male)
	{
		...
		$error="Thêm thất bại";
			$sql="INSERT INTO qq_story (Name,NameOther,story_Status,Content,ImgAvatar,Badge,Waning,Author,Genre,NameEncodeGenres,DateUpload) VALUES ('$Name1','$NameOther1','$Status','$Content1','$Avatar','$Badge','$Waning','$Author1','$Genre','$NameEncodeGenres','$DateUpload')";
			mysqli_query($this->_conn, $sql);	
		  if(mysqli_affected_rows($this->_conn)==1)
			$error="Thêm thành công";
		return $error;
	...
	function UpdateStory($Id,$Name,$NameOther,$Status,$Content,$Avatar,$Badge,$Waning,$Author,$Genre,$NameEncodeGenres,$Country,$URL1,$URL2,$female,$male)
	{
		...
		$Author1=mysqli_real_escape_string($this->_conn,$Author);
		     $error="Sửa thất bại";
$sql="UPDATE qq_story SET Name='$Name1',NameOther='$NameOther1',story_Status='$Status',Content='$Content1',ImgAvatar='$Avatar',Badge='$Badge',Waning='$Waning',Author='$Author1',Genre='$Genre',NameEncodeGenres='$NameEncodeGenres' WHERE Id='$Id'";	
			mysqli_query($this->_conn, $sql);	          
			if(mysqli_affected_rows($this->_conn)==1)
			$error="Sửa thành công";
	...
	function AddChap($Name,$Summary,$DateUpload,$IdStory,$Path,$Title)
	{
		$Title1=mysqli_real_escape_string($this->_conn,$Title);
		$error="Thêm thất bại";
$sql="INSERT INTO qq_chapter (Name,Summary,DateUpload,IdStory,Path,Title) VALUES ('$Name','','$DateUpload','$IdStory','$Path','$Title1')";			
			mysqli_query($this->_conn, $sql);	
		   if(mysqli_affected_rows($this->_conn)==1)
			$error="Thêm thành công";
		return $error;
	}
	function UpdateChap($IdChap,$Name,$Summary,$IdStory,$Path,$Title,$DateUpload)
	{
		$Title1=mysqli_real_escape_string($this->_conn,$Title);
		     $error="Sửa thất bại";
$sql="UPDATE qq_chapter SET Name='$Name',Summary='',IdStory='$IdStory',Path='$Path',Title='$Title1',DateUpload='$DateUpload' WHERE Id='$IdChap'";
			mysqli_query($this->_conn, $sql);	
	        if(mysqli_affected_rows($this->_conn)==1)
			$error="Sửa thành công";
		 //$error=$sql;
